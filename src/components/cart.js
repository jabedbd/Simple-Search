    ...
    render (){
        ...
        if(this.props.state.items.length > 0){
           ...
           data = this.props.state.items.map(r => (
               ...
               <div className="itemimg"><ImageLoading imageUrl={r._source.images[0]}  /></div>
               <div className="itemname">{r._source.title}</div>
               <div className="itemprice">৳{r._source.price}</div>
                <div className="remove" onClick={this.props.removeitem.bind(this,index)} key={index++}><i className="fa fa-times"></i></div>
               </div>
                ))
